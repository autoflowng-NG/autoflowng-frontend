...
export function PricingSection() {
  // FIX 1: default to false so monthly prices show on first load
  ...
  function getPrice(planId: "basic" | "pro" | "business"): string {
    ...
    // 2. Static fallback — always show base monthly price
    // FIX 2: removed annual discount from static fallback (was causing wrong prices)
  ...
  return (
    <section id="pricing" ref={sectionRef} className="relative py-32 lg:py-40">
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        {/* Pricing cards */}
        {/* FIX 3: grid-cols-3 always (was lg:grid-cols-3, causing single column on mobile) */}
            ...
            {plans.map((plan, index) => (
              ...
              >
                ...
                <div className="p-3 md:p-8 lg:p-10">
                  {/* Plan header */}
                  <div className="mb-4 md:mb-8 pb-4 md:pb-8 border-b border-primary/10">
                    ...
                    <p className="hidden md:block text-sm text-muted-foreground mt-2">{plan.description}</p>
                  </div>

                  {/* Price */}
                  <div className="mb-4 md:mb-8">
                    {isLoading ? (
                      <div className="h-8 md:h-14 w-24 md:w-36 bg-muted/30 rounded animate-pulse" />
                    ) : (
                      <>
                        <div className="flex items-baseline gap-1 md:gap-2">
                          <span className="text-lg md:text-5xl lg:text-6xl font-display">
                        ...
                        {!isAnnual && (
                          <p className="text-[10px] md:text-xs text-muted-foreground mt-2 font-mono">
                            billed monthly
                          </p>
                        )}
                      </>
                  ...
                  {/* Features */}
                  <ul className="space-y-1 md:space-y-3 mb-4 md:mb-10">
                    {plan.features.map((feature) => (
                      <li key={feature} className="flex items-start gap-1 md:gap-3">
                        <Check
                  ...
                  {/* CTA */}
                  <button
                    className={`w-full py-2 md:py-4 flex items-center justify-center gap-1 md:gap-2 text-[10px] md:text-sm font-medium transition-all group ${
                      plan.highlight
