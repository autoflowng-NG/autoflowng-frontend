...
function PaymentModal({ plan, onClose, onSuccess }: any) {
  ...
  const initPayment = async () => {
    ...
    try {
      ...
      // It also calls Paystack's /transaction/initialize and returns authorization_url.
      ...
      if (res?.authorization_url) {
        // Preferred path: backend returned a real Paystack checkout URL
        setStep('redirecting');
        setTimeout(() => {
          window.location.href = res.authorization_url;
        }, 600);
      } else if (res?.reference && (window as any).PaystackPop) {
        // Fallback: backend returned only a reference (e.g. PAYSTACK_SECRET_KEY not set in env)
        // Use Paystack inline JS if available on the page
        setStep('redirecting');
        ...
        // Last resort: redirect to Paystack standard checkout with reference
        setStep('redirecting');
        setTimeout(() => {
          window.location.href = `https://checkout.paystack.com/${res.reference}`;
        }, 600);
      } else {
    ...
    <div style={{ padding: "32px", maxWidth: 1200, margin: "0 auto" }}>
      <Reveal>
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          ...
          {detectedCurrency && (
            <p style={{ fontSize: 12, color: "rgba(232,238,255,0.35)", fontFamily: "'DM Mono',monospace", marginTop: 14 }}>
