...
function PaymentModal({ plan, onClose, onSuccess }: any) {
  ...
  const initPayment = async () => {
    setLoading(true);
    try {
      const res: any = await paymentsAPI.initialize({ plan: plan.id, email: email || user?.email, amount: plan.price });
      if (res?.authorization_url) window.open(res.authorization_url, "_blank");
      else if (res?.paymentUrl) window.open(res.paymentUrl, "_blank");
      onSuccess();
    } catch (e: any) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999, padding: 24 }}>
      <div className="af-glass" style={{ borderRadius: 20, padding: "32px", maxWidth: 440, width: "100%", position: "relative", border: `1px solid ${plan.color}30` }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          ...
          <p style={{ fontSize: 28, fontWeight: 900, color: plan.color, fontFamily: "'Syne',sans-serif", marginTop: 8 }}>{plan.priceLabel}<span style={{ fontSize: 14, fontWeight: 500, color: "rgba(232,238,255,0.4)" }}>{plan.period}</span></p>
        </div>
        <div style={{ marginBottom: 20 }}>
          ...
          <input data-testid="input-payment-email" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder={user?.email || "your@email.com"} style={{ width: "100%", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 10, padding: "11px 14px", color: "#E8EEFF", fontSize: 14, fontFamily: "'DM Sans',sans-serif", outline: "none", boxSizing: "border-box" }} />
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onClose} style={{ flex: 1, background: "transparent", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 10, padding: "11px", color: "rgba(232,238,255,0.5)", fontSize: 14, cursor: "pointer", fontFamily: "'DM Sans',sans-serif" }}>Cancel</button>
          <button data-testid="button-pay" onClick={initPayment} disabled={loading} style={{ flex: 2, background: plan.color, border: "none", borderRadius: 10, padding: "11px", color: plan.id === "free" ? "#E8EEFF" : "#04060F", fontSize: 14, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", fontFamily: "'DM Sans',sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
            <CreditCard size={15} /> {loading ? "Processing…" : "Pay with Paystack"}
          </button>
        </div>
      </div>
    </div>
...
export default function Plans() {
  ...
  // Geo-detect on mount, then fetch regional pricing
  useEffect(() => {
  ...
  // Build PLANS with regional prices (fallback to NGN hardcoded if API not ready)
  const PLANS = PLAN_DEFS.map(def => {
    ...
    if (def.id === "trial") {
      priceLabel = t('plans.trial_label');
      period = "";
    } else if (regional) {
      ...
      // API unavailable — do not show hardcoded prices for any currency
      priceLabel = "—";
      period = "";
    }

    return { ...def, priceLabel, period };
  });
              ...
              <Tilt max={4}>
                <div style={{ borderRadius: 20, padding: "28px 22px", position: "relative", overflow: "hidden", background: isPopular ? "rgba(12,17,32,0.95)" : "rgba(8,11,22,0.72)", backdropFilter: "blur(20px)", border: `1.5px solid ${isCurrent ? plan.color : isPopular ? `${plan.color}40` : "rgba(255,255,255,0.07)"}`, boxShadow: isPopular ? `0 0 60px ${plan.color}18` : undefined, height: "100%" }}>
                  ...
                  <button data-testid={`plan-select-${plan.id}`} disabled={isCurrent} onClick={() => !isCurrent && setSelectedPlan(plan)} style={{ width: "100%", background: isCurrent ? "rgba(255,255,255,0.04)" : isPopular ? plan.color : `${plan.color}18`, border: `1px solid ${isCurrent ? "rgba(255,255,255,0.07)" : `${plan.color}40`}`, borderRadius: 12, padding: "11px 0", color: isCurrent ? "rgba(232,238,255,0.4)" : isPopular ? "#04060F" : plan.color, fontSize: 13, fontWeight: 700, cursor: isCurrent ? "default" : "pointer", fontFamily: "'DM Sans',sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, transition: "all 0.18s" }}>
                    {isCurrent ? t('plans.current_plan') : plan.id === "trial" ? t('plans.start_trial') : <><ChevronRight size={14} /> {t('common.upgrade')}</>}
                  </button>
                </div>
              </Tilt>
            </Reveal>
