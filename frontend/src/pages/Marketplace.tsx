...
function PlatformIcon({ name, color }: { name: string; color: string }) {
  // Real brand icons using emoji as a higher-fidelity stopgap per prompt instructions
  // since lucide doesn't have brand logos and we want consistency with Connections page.
  const ICONS: Record<string, string> = {
    youtube: "🔴", facebook: "🔵", instagram: "📸", tiktok: "🎵", whatsapp: "📱",
    telegram: "✈️", discord: "👾", google_sheets: "📊", google_drive: "📁", gmail: "📧",
    salesforce: "☁️", postgresql: "🐘", mysql: "🐬", mongodb: "🍃", redis: "🟥",
    webhook: "🔗", http_request: "🌐",
  };
  return <span style={{ fontSize: 18, color, filter: "drop-shadow(0 0 8px rgba(0,0,0,0.3))" }}>{ICONS[name] || name.slice(0, 2).toUpperCase()}</span>;
}
