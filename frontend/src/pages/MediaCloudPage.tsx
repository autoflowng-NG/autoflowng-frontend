...
import {
  ...
  Plus, ExternalLink, RotateCcw, CalendarClock, Bot,
} from 'lucide-react';
...
const TAB_DEFS: { id: Tab; label: string; icon: any; color: string }[] = [
  ...
  { id: 'publishing', label: 'Campaign Agent', icon: Bot,          color: C.amber  },
...
function CampaignAnalyticsPanel() {
  ...
  const toggleExpand = async (campaignId: string) => {
    ...
    if (drillData[campaignId]) return; // already loaded
  ...
  return (
    <div style={{ marginBottom: 32 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        ...
        <div>
          <p style={{ color: C.text, fontWeight: 700, fontSize: 14, margin: 0, fontFamily: FONT_HEAD }}>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          sub="Campaign Agent jobs will populate metrics here once they run. Launch a campaign to see performance."
        />
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {data.campaigns.map((camp: any) => {
            ...
            const loading = drillLoading[String(camp.campaign_id)];

            const statusColor = camp.campaign_status === 'active'  ? C.green
                              : camp.campaign_status === 'paused'  ? C.amber
                              : camp.campaign_status === 'failed'  ? C.red
                              : C.muted;

            return (
              <div key={camp.campaign_id} style={{
                background: C.surface, border: `1px solid ${isOpen ? C.borderH : C.border}`,
                borderRadius: 12, overflow: 'hidden',
              }}>
                {/* Row */}
                <div
                  onClick={() => toggleExpand(String(camp.campaign_id))}
                  style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', cursor: 'pointer' }}
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
                      <span style={{ color: C.text, fontWeight: 700, fontSize: 13.5, fontFamily: FONT_HEAD,
                                     overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 260 }}>
                        {camp.campaign_name}
                      </span>
                      <span style={{
                        fontSize: 10, fontFamily: FONT_MONO, padding: '2px 8px',
                        borderRadius: 20, background: `${statusColor}18`, color: statusColor,
                      }}>{camp.campaign_status}</span>
                    </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                    <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                      {(camp.platforms ?? []).map((p: string) => (
                        <span key={p} style={{
                          fontSize: 9.5, fontFamily: FONT_MONO, padding: '2px 7px', borderRadius: 20,
                ...
                {/* Drill-down */}
                {isOpen && (
                  <div style={{ borderTop: `1px solid ${C.border}`, padding: '16px 16px 16px' }}>
                    {loading ? (
                      <div style={{ display: 'flex', justifyContent: 'center', padding: 16 }}><Spinner /></div>
                    ) : drill?.by_platform?.length ? (
                      <>
                        <p style={{ color: C.faint, fontSize: 10.5, fontFamily: FONT_MONO, margin: '0 0 10px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                          {drill.by_platform.map((p: any) => (
                            <div key={p.platform} style={{
                              ...
                              background: C.raised, borderRadius: 8, padding: '10px 14px',
                            }}>
                              <span style={{
                                color: C.amber, fontFamily: FONT_MONO, fontSize: 11,
                                fontWeight: 700, minWidth: 90,
                              }}>{p.platform.toUpperCase()}</span>
                              ...
                              <span style={{ color: C.faint, fontSize: 9.5, fontFamily: FONT_MONO, flexShrink: 0 }}>
                                {p.data_points} pts
                              </span>
                            </div>
                          ))}
                        ...
                        {drill.best_day && (
                          <p style={{ color: C.faint, fontSize: 11, fontFamily: FONT_MONO, margin: '10px 0 0' }}>
                            Best day: {drill.best_day.date} — {fmtNum(drill.best_day.views)} views
                          </p>
                        )}
                      </>
                    ) : (
                      <p style={{ color: C.faint, fontSize: 12, fontFamily: FONT_BODY, margin: 0 }}>
                        No platform data for this window.
                      </p>
                    )}
                  </div>
                )}
...
function PublishingGateTab() {
  ...
  const loadJobs = useCallback(async () => {
    setLoading(true);
    const params = new URLSearchParams({ page: String(page), limit: '15', source: 'campaign_agent' });
