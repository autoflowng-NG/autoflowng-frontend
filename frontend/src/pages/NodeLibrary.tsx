...
import { Search, Zap, MousePointerClick, ChevronRight, Copy, Check, Lock } from "lucide-react";
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            padding: "11px 14px",
                            background: unavailable ? "rgba(255,255,255,0.01)" : "rgba(255,255,255,0.025)",
                            border: `1px solid ${unavailable ? "rgba(255,255,255,0.04)" : "rgba(255,255,255,0.06)"}`,
                            borderRadius: 10,
                            transition: "border-color 0.15s",
                            opacity: unavailable ? 0.55 : 1,
                            cursor: unavailable ? "not-allowed" : "default",
                          }}
                          onMouseEnter={e => { if (!unavailable) (e.currentTarget as HTMLElement).style.borderColor = "rgba(255,255,255,0.12)"; }}
                          onMouseLeave={e => { if (!unavailable) (e.currentTarget as HTMLElement).style.borderColor = "rgba(255,255,255,0.06)"; }}
                        >
                          <span style={{ fontSize: 9, fontWeight: 700, fontFamily: "'DM Mono',monospace", padding: "2px 7px", borderRadius: 5, flexShrink: 0, background: node.kind === "trigger" ? "rgba(251,191,36,0.12)" : unavailable ? "rgba(255,255,255,0.05)" : "rgba(0,200,150,0.1)", color: node.kind === "trigger" ? "#FBBF24" : unavailable ? "rgba(232,238,255,0.25)" : "#00C896" }}>
                            {node.kind === "trigger"
                              ? <><MousePointerClick style={{ display: "inline", verticalAlign: "text-bottom", marginRight: 3 }} size={9} />TRIGGER</>
                              : <><Zap style={{ display: "inline", verticalAlign: "text-bottom", marginRight: 3 }} size={9} />ACTION</>
                            }
                          </span>
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                            {unavailable ? (
                              <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 10, fontWeight: 700, fontFamily: "'DM Mono',monospace", color: "rgba(232,238,255,0.25)", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 6, padding: "2px 8px" }}>
