...
import { connectionsAPI, tokenStore, googleDriveAPI } from "../lib/api";
...
import { Link2, Check, ExternalLink, Trash2, Plus, Phone, MessageSquare, Webhook, Globe, Zap, HardDrive } from "lucide-react";
import { PlatformSVGIcon } from "../components/PlatformIcons";

// ── Google Drive integration card (Phase 3A) ─────────────────────────────────
  ...
  return (
    <Reveal>
      <div style={{
        ...
        borderRadius: 14, padding: 18, display: "flex", flexDirection: "column", gap: 12, height: "100%",
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{
            width: 40, height: 40, borderRadius: 10, background: "rgba(66,133,244,0.12)",
            border: "1px solid rgba(66,133,244,0.3)", display: "flex", alignItems: "center", justifyContent: "center",
          }}>
            <HardDrive size={18} color="#4285F4" />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 800, color: "#E8EEFF", fontFamily: "'Syne',sans-serif" }}>Google Drive</div>
            <div style={{ fontSize: 11, color: "rgba(232,238,255,0.45)" }}>Save generated media to your Drive</div>
          </div>
          {status.connected && (
            <span style={{
              fontSize: 10, color: "#00C896", background: "rgba(0,200,150,0.1)",
              border: "1px solid rgba(0,200,150,0.25)", borderRadius: 6,
              padding: "3px 8px", fontFamily: "'DM Mono',monospace", letterSpacing: "0.06em",
            }}>CONNECTED</span>
          )}
        </div>
        {status.connected ? (
          <button onClick={disconnect} disabled={loading} style={{
            ...
            color: "#FB7185", borderRadius: 10, padding: "9px 12px", fontSize: 12, fontWeight: 700,
            cursor: loading ? "wait" : "pointer", fontFamily: "'DM Mono',monospace",
          }}>{loading ? "Working…" : "Disconnect"}</button>
        ) : (
          <button onClick={connect} disabled={loading} style={{
            background: "rgba(66,133,244,0.1)", border: "1px solid rgba(66,133,244,0.3)",
            color: "#4285F4", borderRadius: 10, padding: "9px 12px", fontSize: 12, fontWeight: 700,
            cursor: loading ? "wait" : "pointer", fontFamily: "'DM Mono',monospace",
          }}>{loading ? "Working…" : "Connect Google Drive"}</button>
        )}
      </div>
    </Reveal>
  );
...
const PLATFORMS = [
  { id: "whatsapp", label: "WhatsApp",  color: "#25D366", desc: "Send and receive WhatsApp messages", fields: [{ name: "phoneNumberId", label: "Phone Number ID", type: "text" }, { name: "accessToken", label: "Access Token", type: "password" }, { name: "verifyToken", label: "Verify Token", type: "text" }] },
  { id: "telegram", label: "Telegram",  color: "#229ED9", desc: "Connect your Telegram bot", fields: [{ name: "botToken", label: "Bot Token", type: "password" }, { name: "chatId", label: "Chat ID", type: "text" }] },
  { id: "google",   label: "Google",   color: "#4285F4", desc: "Google Calendar, Sheets, Drive", fields: [] },
  { id: "github",   label: "GitHub",   color: "#E8EEFF", desc: "Repository events and webhooks", fields: [] },
  { id: "slack",    label: "Slack",    color: "#4A154B", desc: "Send messages to Slack channels", fields: [] },
  { id: "youtube",  label: "YouTube",  color: "#FF0000", desc: "Manage YouTube content", fields: [] },
  { id: "facebook", label: "Facebook", color: "#1877F2", desc: "Facebook Graph API", fields: [] },
  { id: "webhook",  label: "Webhook",  color: "#00C896", desc: "Custom webhook endpoint", fields: [{ name: "url", label: "Webhook URL", type: "url" }, { name: "secret", label: "Secret", type: "password" }] },
];
    ...
    <Reveal>
      <div className="af-glass" data-testid={`connection-card-${p.id}`} style={{ borderRadius: 16, padding: "20px", position: "relative", overflow: "hidden", border: `1px solid ${connected ? "rgba(0,200,150,0.15)" : "rgba(255,255,255,0.06)"}`, transition: "border-color 0.2s" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 16 }}>
          <div style={{ width: 44, height: 44, borderRadius: 10, background: `${p.color}15`, border: `1px solid ${p.color}30`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <PlatformSVGIcon id={p.id} size={24} />
          </div>
          <div style={{ flex: 1 }}>
