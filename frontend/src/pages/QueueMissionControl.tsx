...
function StarvationRiskPanel({ starvation }: { starvation: any }) {
  ...
  const lvlColor = { none: "#10B981", medium: "#F59E0B", high: "#FF8C42", critical: "#FF4D6D" }[riskLevel] ?? "#6B7280";
      ...
      {risks.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {risks.map((risk: any, i: number) => {
            const sevColor = { critical: "#FF4D6D", high: "#FF8C42", medium: "#F59E0B" }[risk.severity] ?? "#7C6FFF";
            return (
