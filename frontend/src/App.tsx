...
function SharedExecutionPageRoute() {
  const { token } = useParams<{ token: string }>();
  return <SharedExecutionPage token={token ?? ""} />;
}

function ExecutionPageRoute() {
  const { runId } = useParams<{ runId: string }>();
  return <ExecutionPage runId={runId ?? ""} />;
}
    ...
    <BrowserRouter>
      ...
      <Suspense fallback={<Spinner />}>
        ...
        <Routes>
          ...
          {/* Phase 10G */}
          <Route path="/share/execution/:token" element={<SharedExecutionPageRoute />} />
