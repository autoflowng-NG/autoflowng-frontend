...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const checkAuth = async (retry = true) => {
      try {
        ...
        if (err?.status === 401) {
          ...
          // Retry once after 1.5s for transient/cold-start errors
          setTimeout(() => checkAuth(false), 1500);
        } else {
          // If second attempt also fails (non-401), keep token but stop loading
          setIsLoading(false);
        }
      }
    };
