...
export function PricingSection() {
  ...
  function getPrice(planId: string): string {
    ...
    if (!base) return "—";
    return `${fb.symbol}${base.toLocaleString()}`;
  }
            ...
            >
              ...
              <div className="p-3 md:p-8 flex-1">
                ...
                <div className="flex items-center gap-1 md:gap-3 mb-3 md:mb-6">
                  <div className="text-base md:text-2xl">{plan.icon}</div>
                  <p
                ...
                <div className="mb-6">
                  {isLoading ? (
                    ...
                    <div className="flex items-end gap-1">
                      <span className="text-lg md:text-4xl font-bold text-foreground font-display">
                        {getPrice(plan.id)}
                      </span>
                ...
                {/* Features */}
                <ul className="space-y-1 md:space-y-3 mb-4 md:mb-8">
                  {plan.features.map((feature) => (
                    <li
                      key={feature}
                      className="flex items-start gap-1 md:gap-2.5 text-[10px] md:text-sm text-muted-foreground"
                    >
                      <Check
              ...
              {/* CTA */}
              <div className="p-3 md:p-8 pt-0 mt-auto">
                <button
                  onClick={() => router.push("/register")}
                  className={`w-full py-2 md:py-3.5 rounded-xl flex items-center justify-center gap-1 md:gap-2 text-[10px] md:text-sm font-semibold transition-all group ${
                    plan.popular
                      ? "hover:opacity-90"
